...
while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
    ...
    //count compilation
    $query2 = "SELECT count(*) as compilation FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  WHERE Project.id = " . $row["id"] . " AND Compilation.testpassed = -1 ";
    $result2 = $mysqli->query($query2);
    $row["compilation"] = $result2->fetch_assoc()["compilation"];

    //count teste executed
    $query3 = "SELECT count(*) as test FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  WHERE Project.id = " . $row["id"]. " AND Compilation.testpassed <> -1 ";
    $result3 = $mysqli->query($query3);
    $row["test"] = $result3->fetch_assoc()["test"];
    ...
    //count compilation with error
    $query5 = "SELECT count(*) as complitaion FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  WHERE Project.id = '" . $row["id"] . "' AND Compilation.typeError <> 'no-error' AND Compilation.testpassed = -1 ";
