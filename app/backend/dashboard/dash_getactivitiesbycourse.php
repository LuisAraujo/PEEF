...
if(getcurrentcourse_session() == "-1"){
    ...
    while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
        ...
        $query2 = "SELECT count(*) as compilation FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  INNER JOIN Activity ON Activity.id = project.Activity_id WHERE Activity.id = " . $row["id"]. " AND Compilation.testpassed = -1";
        ...
        $query5 = "SELECT count(*) as complitaion FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  INNER JOIN Activity ON Activity.id = project.Activity_id WHERE Activity.id = '" . $row["id"] . "' AND Compilation.typeError <> 'no-error' AND Compilation.testpassed = -1";
