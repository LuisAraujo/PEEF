        ...
        gerateFileCode(iduser, token, function(token, filename){
           // console.log(filename);
            ...
            internalCheckOut = setInterval(
                function(){ checkOutput(token)} , timeCheckOut
            );
        });
    })
    ...
    $.ajax({
        ...
        if(noutput < lines.length-1) {
            ...
            if (lines[localnoutput] == "__close") {
                ...
                //console.log(lines[localnoutput]);
                lineoutput = lines[localnoutput];
                //replace html symbol
                lineoutput = lineoutput.replace("<", "&lt");
                lineoutput = lineoutput.replace(">", "&gt");
                $("#out_lines").append(lineoutput + "<br>");
            }
        }
        }) .fail(function(data) {
            console.log("erro");
