...
PEEFbot.prototype.start = function () {
    ...
    //this.getSolutions("Syntax Error","Suberror");
...
PEEFbot.prototype.getSolutions = function (id, typeerror, subtypeerror) {
    //check n solution?
    _this = this;
    $.post("../../backend/project/project_getsolution.php",
        {type: typeerror, subtype: subtypeerror} )
        .done(function(msg)
        {

            if ( $("#chat-"+id).length ) {
                $("#chat-"+id).html("<strong>"+msg+"</strong>");
              console.log("criado " + "#chat-"+id);
            }else{

                setTimeout( function(){ _this.getSolutions(id, typeerror, subtypeerror)}, 1000 );
                console.log("n criado");
            }
    ...
    if(response == "no"){
       ...
       return [tempnode.getMessage(), tempnode.type];
        ...
        if(this.currentnode.type == "presentation") {
            ...
            var sl = '<span id="chat-'+val+'">AGUARDE</span>';

            this.getSolutions(val, "", "");
