...
while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
    ...
    $query4 = "SELECT count(*) as passedtest FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  WHERE Project.id = '" . $row["id"] . "' AND Compilation.testpassed = 1 ";
    ...
    $query6 = "SELECT Compilation.typeError as typeerror FROM Compilation INNER JOIN Code ON Compilation.Code_id = Code.id INNER JOIN Project ON Code.Project_id = Project.id  WHERE Project.id = '" . $row["id"] . "' ORDER BY Compilation.date , Compilation.hours DESC LIMIT 1";
    $result6 = $mysqli->query($query6);
    $row["typeerror"] = $result6->fetch_assoc()["typeerror"];

    $query7 = "SELECT Count(*) as unviewed FROM Message WHERE Project_id = '" . $row["id"] . "' AND hasview = 0";
